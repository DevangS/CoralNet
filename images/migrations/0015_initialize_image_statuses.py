...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print (
            "-----\n"
            "This migration initializes boolean statuses for images.  The following assumptions "
            "are made for simplicity:\n\n"
            "(1) We don't need to save status values of any current images.\n"
            "(2) Partially-human-annotated (some but not all points human-annotated) images "
            "currently have disposable annotations, and it's okay to "
            "have a robot overwrite with its annotations. (Note that this should not "
            "happen with the Import Annotations utility, where points and annotations "
            "are imported together.\n"
            "(3) A fully human-annotated image is one where each point has an annotation "
            "by any non-robot user.\n\n"
        )

        # Set status fields for existing images.
        # Note: all fields are added as False by default. We just need
        # to set the ones that are supposed to be True here.
        randomPointsCount = 0
        annotatedByHumanCount = 0

        for image in orm.Image.objects.all():
            status = orm.ImageStatus()

            # (1) If the image has points in the database, mark hasRandomPoints.
            if orm.Point.objects.filter(image=image).count() > 0:
                status.hasRandomPoints = True
                randomPointsCount += 1

                # (2) Additionally, if all the image's points have been human annotated, mark annotatedByHuman.
                if (orm.Point.objects.filter(image=image, annotation=None).count() == 0 and
                    orm.Point.objects.filter(image=image, annotation__user__username="robot").count() == 0):
                    status.annotatedByHuman = True
                    annotatedByHumanCount += 1
        ...
        print (
            "%s images marked as having random points.\n"
            "%s images marked as human-annotated.\n"
            "-----" % (randomPointsCount, annotatedByHumanCount)
        )
    ...
    def backwards(self, orm):
        print (
            "-----\n"
            "This migration rollback doesn't do anything; don't know how to revert image statuses."
            "\n-----"
        )
