...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print (
            "-----\n"
            "This migration sets source-level cm heights for sources that don't\n"
        ...
        numSourcesChanged = 0
        defaultCmHeight = 50
        sourcesWithoutCmHeight = orm.Source.objects.filter(image_height_in_cm__isnull=True)

        for source in sourcesWithoutCmHeight:
            source.image_height_in_cm = defaultCmHeight
            source.save()
            numSourcesChanged += 1

        print (
            "%s sources set with a source-level cm height of %s.\n"
            "-----" % (numSourcesChanged, defaultCmHeight)
        )
    ...
    def backwards(self, orm):
        print (
            ...
            "sources had cm heights set before, so we can't un-set those cm heights.\n"
            "-----"
        )
